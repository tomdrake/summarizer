summarizer.cph = function(df, dependent, explanatory, explanatory.multi=NULL, random_effect=NULL, metrics=FALSE, weights = NULL){
  ...
  # Univariable
  coxphuni.out = coxphuni(df, dependent, explanatory, weights = weights)
  coxphuni.df = fit2df(coxphuni.out, estimate.suffix = " (univariable)")
  
  # Multivariable/Mixed
  if (is.null(explanatory.multi)){
    coxphmulti.out = coxphmulti(df, dependent, explanatory, weights = weights)
  } else {
    coxphmulti.out = coxphmulti(df, dependent, explanatory.multi, weights = weights)
  }
