summarizer.glm = function(df, dependent, explanatory, explanatory.multi=NULL, random_effect=NULL, metrics=FALSE, weights=NULL){
  ...
  if (is.null(random_effect)){
    if (is.null(explanatory.multi)){
      glmmulti.out = glmmulti(df, dependent, explanatory, weights=weights)
    } else {
      glmmulti.out = glmmulti(df, dependent, explanatory.multi, weights=weights)
    }
    glmmulti.df = fit2df(glmmulti.out, metrics=metrics, estimate.suffix = " (multivariable)", weights=weights)
